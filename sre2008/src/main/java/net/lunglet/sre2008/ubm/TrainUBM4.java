...
public final class TrainUBM4 {
    ...
    private static GMMMAPStats trainGMM(final H5File dataFile, final GMM gmm) throws InterruptedException,
            ExecutionException {
    ...
    private static void trainGMMwithFlooring(final H5File dataFile, final DiagCovGMM gmm, final FloatVector varFloor,
            final int maxiter) throws InterruptedException, ExecutionException {
        for (int iter = 1; iter <= maxiter; iter++) {
            ...
            GMMMAPStats stats = trainGMM(dataFile, gmm);
            ...
            float thresh = 2.0f * gmm.getDimension() * 10.0f;
            GMMUtils.replaceWeak(gmm, stats, thresh);
            LOGGER.info("Weights after: " + gmm.getWeights());
            IOUtils.writeGMM("ubm_strong_" + gmm.getMixtureCount() + "_" + iter + ".h5", gmm);
    ...
    public static void main(final String[] args) throws InterruptedException, ExecutionException {
        ...
        for (int i = 2; i <= 32; i++) {
            ...
            int maxiter = 3;
            trainGMMwithFlooring(dataFile, gmm, varianceFloor, maxiter);
        }
        for (int i = 64; i <= 2048; i *= 2) {
            gmm = GMMUtils.splitAll(gmm);
            AssertUtils.assertEquals(i, gmm.getMixtureCount());
            int maxiter = i == 2048 ? 3 : 5;
            trainGMMwithFlooring(dataFile, gmm, varianceFloor, maxiter);
        }
    }
}
