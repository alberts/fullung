...
public final class VADFeatureCombinerTest {
    ...
    public void test() throws UnsupportedAudioFileException, IOException {
        ...
        FeatureSet[] features = htkMfccBuilder.apply(audioStream);
        assertEquals(2, features.length);

        FeatureSet[] phnRecVADFeatures = new FeatureSet[features.length];
        ...
        FeatureSet[] squelchFeatures = vad.apply(features);

        FeatureCombiner featureCombiner = new VADFeatureCombiner();
        FeatureSet[] finalFeatures = new FeatureSet[features.length];
        for (int i = 0; i < features.length; i++) {
